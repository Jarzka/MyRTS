...
/** This class is used to store and process player inputs during gameplay. **/
public class MultiplayerSynchronizationManager {
    ...
    /** SimTick is a "communication turn" used for network communication.
     * 1 simTick = 5 world update ticks by default.
     * When a new SimTick is reached, the game executes players input information.
     * If such information is not available, we wait for it.
     */
    ...
    public boolean handleNewSimTick() {
        ...
        /* Check that we have all the needed input information for the next SimTick so that we can
        * continue executing the simulation. If the input is not available for all players,
        * isWaitingInputForNextSimTick remains true.
        * The input for the next SimTick was sent during the previous SimTick. */
